    ...
    protected BufferedImage baseFonts() {
        ...
        for (int i = 0; i < 16; i++) {
            for (int j = 0; j < 16; j++) {
                ...
                image.setRGB(2 * j, 2 * i, (pixels[3]) ? 0xffffffff : 0xff000000);
                image.setRGB(2 * j + 1, 2 * i, (pixels[2]) ? 0xffffffff : 0xff000000);
                image.setRGB(2 * j, 2 * i + 1, (pixels[1]) ? 0xffffffff : 0xff000000);
                image.setRGB(2 * j + 1, 2 * i + 1, (pixels[0]) ? 0xffffffff : 0xff000000);
            }
        }
        return image;
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ImageIO.write(ac.createFonts(1, ac.baseFonts()), "png",
                    new FileImageOutputStream(new File("pFrame_2x2.png")));
            ImageIO.write(ac.createFonts(2, ac.baseFonts()), "png",
                    new FileImageOutputStream(new File("pFrame_4x4.png")));
            ImageIO.write(ac.createFonts(3, ac.baseFonts()), "png",
                    new FileImageOutputStream(new File("pFrame_6x6.png")));
            ImageIO.write(ac.createFonts(4, ac.baseFonts()), "png",
                    new FileImageOutputStream(new File("pFrame_8x8.png")));
        } catch (Exception e) {
